...
Selecting a dataset to work individually and inspecting missing values
...
mean_replace <- function(arg){
  ...
  print(paste0('Percentage of missing repalced: ',round(j/length(l),2),'%'))
  return(l)
}
...
lag <- round(dim(pol)[1]/2)
data <- pol$PM2.5

X <- array(data[1:(length(data) - lag)], dim = c(length(data) - lag, lag, 1))
y <- data[(lag + 1):length(data)]
len <- dim(X)[1]

# Split into training and test sets
train_size <- round(len * 0.5)
...
plot((train_size + lag + 1):365, y_test, type = 'l', col = 'blue', lwd = 2,
     main = "Actual vs Predicted", xlab = "Time", ylab = "Value")
